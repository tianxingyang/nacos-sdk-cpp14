...
class NacosException : public std::exception
{
 ...
 public:
  ...
  const char* what() const NACOS_NOTHROW()
  {
    return _errmsg.c_str();
  };

  const int errorcode() const NACOS_NOTHROW()
  {
    return _errcode;
  };
...
class NetworkException : public std::exception
{
 ...
 public:
  NetworkException(int errorcode, const char* errormsg) NACOS_NOTHROW()
      : _curlerrcode(errorcode), _errmsg(errormsg){};

  NetworkException(int errorcode, const NacosString& errormsg) NACOS_NOTHROW()
      : _curlerrcode(errorcode), _errmsg(errormsg){};
...
{
 public:
  IOException(int errorcode, const char* errormsg) NACOS_NOTHROW()
      : NacosException(errorcode, errormsg){};

  IOException(int errorcode, const NacosString& errormsg) NACOS_NOTHROW()
      : NacosException(errorcode, errormsg){};
...
{
 public:
  MalformedConfigException(const NacosString& file,
                           const NacosString& detailedMsg)
      : NacosException(
            NacosException::MALFORMED_CONFIG_FILE,
...
{
 public:
  InvalidFactoryConfigException()
      : NacosException(NacosException::INVALID_FACTORY_CONFIG,
